...
const CardVideo = ({ props }) => {
    const { overview, title, poster_path, original_title, popularity,
        release_date } = props;
    return (
        <Link to={`/videos/preview/${original_title}`} state={{poster_path, overview, popularity,release_date }} className="relative flex flex-col">
...
const CardTv = ({ props }) => {
    ...
    return (
            <Link to={`/videos/preview/${original_name}`} state={{poster_path, overview, popularity}} className="relative flex flex-col">
                <div className="absolute inset-0 bg-center rounded-xl dark:bg-black"></div>
                <div className="group relative m-0 flex  w-44 rounded-xl shadow-xl ring-gray-900/5 sm:mx-auto sm:max-w-lg">
                    <div className="z-10 h-full w-full overflow-hidden rounded-xl border border-gray-200 opacity-80 transition  duration-300 ease-in-out group-hover:opacity-100 dark:border-gray-700 dark:opacity-70">
                    <img src={`https://image.tmdb.org/t/p/w500${poster_path}`} alt={title} />
