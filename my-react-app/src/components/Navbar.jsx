...
const Navbar = () => {
  ...
  const [setIsGenreOpen] = useState(false);

  return (
    <>
      <header className='border-b-2 border-b-white top-0 left-0 w-full bg-gray-950 py-3 px-6 sm:px-10 font-[sans-serif] min-h-[70px] tracking-wide z-50'>
        <div className='flex flex-wrap items-center justify-between gap-4 w-full'>
          ...
          <div id="collapseMenu" className="max-lg:hidden lg:!flex lg:items-center max-lg:before:fixed max-lg:before:bg-black max-lg:before:opacity-40 max-lg:before:inset-0 max-lg:before:z-50">
            ...
            <ul className='flex lg:!flex lg:gap-x-6 max-lg:space-y-3 max-lg:fixed max-lg:bg-[#151d20] max-lg:w-2/3 max-lg:min-w-[300px] max-lg:top-0 max-lg:left-0 max-lg:px-10 max-lg:py-4 max-lg:h-full max-lg:shadow-md max-lg:overflow-auto z-50'>
              <li className='mb-6 hidden max-lg:block'>
                <a href=""><img src="https://readymadeui.com/readymadeui-light.svg" alt="logo" className='w-36' /></a>
              </li>
              <li className='mb-6 hidden max-lg:block'>
            <a href=""><img src="https://readymadeui.com/readymadeui-light.svg" alt="logo"
                className='w-36' />
            </a>
          </li>
          <li className='max-lg:border-b max-lg:py-3 px-3'>
            < Link to='/home' className='text-gray-300 hover:text-white text-base flex items-center'>
              ...
              <li className='relative max-lg:border-b max-lg:py-3 px-3' onMouseEnter={() => setIsSeriesOpen(true)} onMouseLeave={() => setIsSeriesOpen(false)}>
                <Link to="/series" className='text-gray-300 hover:text-white text-base flex items-center'>
                ...
                {isSeriesOpen && (
                  <ul className='absolute bg-gray-800 text-gray-300 mt-2 rounded shadow-lg'>
                    <li><Link to="/series/action" className='block px-4 py-2 hover:bg-gray-700'>Action</Link></li>
                    <li><Link to="/series/drama" className='block px-4 py-2 hover:bg-gray-700'>Drama</Link></li>
                    <li><Link to="/series/comedy" className='block px-4 py-2 hover:bg-gray-700'>Comedy</Link></li>
                  </ul>
                )}
              </li>

              <li className='relative max-lg:border-b max-lg:py-3 px-3' onMouseEnter={() => setIsGenreOpen(true)} onMouseLeave={() => setIsGenreOpen(false)}>
                <Link to="/genre/pilihgenre" className='text-gray-300 hover:text-white text-base flex items-center'>
          ...
          <div className='flex gap-3 max-w-max'>
            <div className='flex items-center max-lg:ml-auto'>
              <input type="text" placeholder="Search something..."
                className="bg-transparent border border-gray-300 text-sm w-full px-5 text-gray-300 rounded-full py-2 outline-none" />
